...
export class ResponseBoardMapper {
  ...
  static map(board: BoardEntity): ResponseBoardMapper {
    return new ResponseBoardMapper({
      ...
      lists: board.lists && board.lists.map(ResponseListsMapper.map),
      labels: board.labels && ResponseLabelMapper.mapAll(board.labels),
      members: board.members.map(ResponseMemberMapper.map),
    });
  }
}
